...
import com.example.simplecalendar.objects.States.isUpdatingCalDAV
...
//        try {
//            val calDAVCalendars = context.config.caldavSyncedCalendarIds?.let { getCalDAVCalendars(it, showToasts) }
//            if (calDAVCalendars != null) {
...
//        val eventRemoteID = newUri?.lastPathSegment?.let { java.lang.Long.parseLong(it) }
//        event.importId = eventRemoteID?.let { getCalDAVEventImportId(calendarId, it) }.toString()
//
